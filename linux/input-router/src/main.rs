//! beaglewing-router: the Linux side of Beaglewing, one binary with
//! several roles (see `usage`): the input router, the clipboard agent,
//! the status icon, and a few diagnostics. docs/input-routing.md and
//! docs/clipboard.md describe the design.
mod capture;
mod clipboard;
mod geometry;
mod hidmap;
    ...
    eprintln!(
        "usage: beaglewing-router <command>\n\
         ...
         commands:\n\
           ...
           tray                       status icon with service controls\n\
...
fn cmd_run(args: &[String]) {
    ...
    let result = router::run(cfg, tx, rx);
    // Never leave a stale state file behind for the tray to misread.
    let _ = std::fs::remove_file(router::state_path());
    if let Err(e) = result {

...
fn observe() -> (RouterState, bool, bool) {
    let router = if !systemctl(&["is-active", "--quiet", "beaglewing-router"]) {
            ...
            .trim()
        {
            ...
            "local" => RouterState::Local,
            // "consent", or no file yet: the router is still starting up
            _ => RouterState::Consent,
        }
    };
    (
        router,
...
fn draw_icon(state: RouterState, size: i32) -> Icon {
    let (r, g, b) = match state {
        ...
        RouterState::Consent => (0xE6, 0xB4, 0x2A),
...
fn describe(state: RouterState) -> &'static str {
    match state {
        ...
        RouterState::Consent => "Router waiting for the consent dialog",
...
pub fn run() -> ! {
    ...
    // Only push an update when something changed: every update makes the
    // shell rebuild the icon texture on its main thread, and doing that
    // every second was enough to jank input delivery.
    let mut last = (router, router_at_login, clipboard_running);
    loop {

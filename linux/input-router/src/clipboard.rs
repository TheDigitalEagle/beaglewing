...
use clipframe::{
    ...
    KIND_TAR, KIND_TEXT, MAX_TRANSFER, RATE_TO_LINUX, RATE_TO_WINDOWS, T_ABORT, T_PING, T_PONG,
};

/// Xwayland gets a new auth cookie whenever it restarts (re-login, a
/// crash), which silently breaks any long-lived process holding the old
/// XAUTHORITY. Resolve the newest mutter auth file on every call.
fn x_env() -> Vec<(String, String)> {
    ...
    if let Some((_, p)) = newest {
        env.push(("XAUTHORITY".to_string(), p.to_string_lossy().into_owned()));
    }
    env
}
...
pub fn run(addr: &str) -> ! {
    ...
    loop {
        ...
        // The relay answers PING itself; 30s without any frame while we
        // are pinging means the relay (or the Pi) is gone, even if TCP
        // has not noticed (a power cut sends no FIN).
        let mut last_rx = Instant::now();
        'session: loop {
            match rx.recv() {
                Ok(Ev::Tick) => {
                    if last_rx.elapsed() > Duration::from_secs(30) {
                        println!("[clip] relay silent for 30s; reconnecting");
                        break 'session;
                    }
                    if writer.write_all(&frame(T_PING, 0, &[])).is_err() {
                        break 'session;
                    }
                }

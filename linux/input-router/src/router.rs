...
/// Set once the main loop runs. Before that we may be blocked in a
/// portal call (waiting on the consent dialog), which cannot observe the
/// flag; exiting outright keeps systemd restarts fast.
static IN_LOOP: AtomicBool = AtomicBool::new(false);

extern "C" fn on_signal(_: libc::c_int) {
    if !IN_LOOP.load(Ordering::SeqCst) {
...
/// Publish the router's state for the tray: "consent" (waiting for the
/// portal dialog), "local", "remote", or "no-bridge". Written to a small
/// file in XDG_RUNTIME_DIR on every transition; readers poll it.
pub fn state_path() -> std::path::PathBuf {
    let dir = std::env::var("XDG_RUNTIME_DIR").unwrap_or_else(|_| "/tmp".into());
    std::path::PathBuf::from(dir).join("beaglewing-router.state")
}
...
pub fn run(cfg: Config, tx: Sender<Event>, rx: Receiver<Event>) -> Result<(), String> {
    ...
    publish_state("consent");
    // The dialog is easy to lose behind windows; make it findable.
    let _ = std::process::Command::new("notify-send")
        .args([
            "--app-name=Beaglewing",
            "--expire-time=15000",
            "Beaglewing needs a click",
            "Approve the input capture dialog to enable edge switching.",
        ])
    ...
    IN_LOOP.store(true, Ordering::SeqCst);
    ...
    // Key delivery latency stats (count, sum us, max us), reported per minute.
    let mut key_lag: (u64, u64, u64) = (0, 0, 0);
    ...
    let go_local = |portal: &InputCapture,
        ...
        *buttons = 0;
        ...
        publish_state(if bridge.is_some() { "local" } else { "no-bridge" });
    ...
    loop {
        ...
        if last_stats.elapsed() > Duration::from_secs(60) {
            ...
            if key_lag.0 > 0 {
                println!(
                    "[router] key delivery lag (compositor -> router), last minute: {} keys, avg {:.1}ms, max {:.1}ms",
                    key_lag.0,
                    key_lag.1 as f64 / key_lag.0 as f64 / 1000.0,
                    key_lag.2 as f64 / 1000.0
                );
                key_lag = (0, 0, 0);
            }
        }
        ...
        match ev {
            ...
            Event::Capture(cev) => {
                ...
                // at the target host.
                // Only pointer motion is continuous enough to judge gaps by;
                // pauses between keystrokes are just typing.
                let is_motion = matches!(cev, CaptureEvent::Motion(..) | CaptureEvent::Frame);
                if events_this_activation && is_motion {
                ...
                match cev {
                    CaptureEvent::Key(code, pressed, ts_us) => {
                        // Delivery lag: compositor timestamp vs now (both
                        // CLOCK_MONOTONIC). Logged when it exceeds 20ms so a
                        // "typing feels laggy" report has a number to check.
                        let now_us = {
                            let mut ts = libc::timespec { tv_sec: 0, tv_nsec: 0 };
                            unsafe { libc::clock_gettime(libc::CLOCK_MONOTONIC, &mut ts) };
                            ts.tv_sec as u64 * 1_000_000 + ts.tv_nsec as u64 / 1000
                        };
                        let lag_us = now_us.saturating_sub(ts_us);
                        if lag_us < 60_000_000 {
                            key_lag.0 += 1;
                            key_lag.1 += lag_us;
                            key_lag.2 = key_lag.2.max(lag_us);
                            if lag_us > 20_000 {
                                println!("[router] key delivered late by compositor: {}ms", lag_us / 1000);
                            }
                        }

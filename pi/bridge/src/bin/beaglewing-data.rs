        ...
        *current.lock().unwrap() = Some(stream.try_clone().expect("clone tcp"));
        ...
        loop {
            match stream.read_exact(&mut buf) {
                Ok(()) => {
                    // Liveness: a PING (type 0x01) from the agent is answered
                    // here with a PONG (0x02) rather than forwarded, so the
                    // agent learns quickly when this relay has gone away
                    // (a Pi power cut leaves its TCP connection a zombie).
                    if buf[0] == 0x01 {
                        let mut pong = [0u8; FRAME];
                        pong[0] = 0x02;

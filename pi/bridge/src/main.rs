...
struct Hid {
    ...
    y: u16,
    hid_ok: bool,
    /// (count, sum us, max us, window start) for keyboard report writes
    kbd_stats: (u64, u64, u64, Instant),
}
...
impl Hid {
    ...
    fn flush_kbd(&mut self) {
        ...
        // A keyboard report write blocks until the host polls the
        // endpoint; anything beyond a few ms means the host is stalling.
        let t0 = std::time::Instant::now();
        self.write_report(Which::Kbd, &r);
        let us = t0.elapsed().as_micros() as u64;
        self.kbd_stats.0 += 1;
        self.kbd_stats.1 += us;
        self.kbd_stats.2 = self.kbd_stats.2.max(us);
        if us > 5000 {
            log(&format!("slow keyboard report write: {}ms (host polling stall)", us / 1000));
        }
        if self.kbd_stats.3.elapsed() > Duration::from_secs(60) {
            log(&format!(
                "keyboard report write time, last minute: {} reports, avg {:.2}ms, max {:.2}ms",
                self.kbd_stats.0,
                self.kbd_stats.1 as f64 / self.kbd_stats.0 as f64 / 1000.0,
                self.kbd_stats.2 as f64 / 1000.0
            ));
            self.kbd_stats = (0, 0, 0, Instant::now());
        }
    }

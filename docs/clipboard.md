...
Silent failure modes fixed 2026-09-17: (1) the agent-relay TCP link had
no heartbeat, so a Pi power cut left the agent writing into a zombie
socket indefinitely; the relay now answers PING itself and the agent
reconnects after 30s of silence. (2) Xwayland issues a new auth cookie
when it restarts (re-login, crash); a long-lived agent holding the old
XAUTHORITY could no longer read or set the clipboard. The agent now
resolves the newest mutter Xwayland auth file on every xclip call.

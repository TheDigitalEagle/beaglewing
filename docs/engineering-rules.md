...
- Never change the network configuration of a machine you can only reach
  over that network (band, BSSID, address, firewall) without a lifeline:
  an automatic revert on a timer, a second path in, or a person at the
  console. A failed change strands the device until someone touches it.

...
- **Full keyboard** with correct modifier handling, plus a configurable
  emergency return hotkey (default LCtrl+LAlt+Backslash). Measured
  per-keystroke overhead on the reference setup: about 5ms typical, under
  15ms worst case (0.1ms compositor handoff, ~4ms Wi-Fi hop, under 2ms
  USB); both ends log per-minute latency statistics to the journal.
...
- **Status icon.** A tray icon (StatusNotifierItem: KDE, and GNOME with
  the AppIndicator extension Ubuntu ships enabled) shows which machine
  has your input, with a menu to start/stop/restart the router, toggle
  start-at-login, fetch an offered file batch, and control clipboard
  sync.
...
| - | Login autostart and status icon (Linux) | ✅ |
| - | Latency instrumentation (per-minute stats in the journal) | ✅ |
| - | Config file, Windows tray icon | planned |
...
systemctl --user enable --now beaglewing-router beaglewing-clipboard beaglewing-tray
```

After that everything starts at login. On GNOME the router asks for one
InputCapture consent click right after login (portal v1 has no
persistence yet); the tray icon shows the state and offers
start/stop/restart, start-at-login, and a manual file pull.
...
sync (input needs nothing at all). To start it with Windows, press
Win+R, run `shell:startup`, and drop a shortcut to the exe in that
folder. Emergency return to local is LCtrl+LAlt+Backslash; stopping the
router always hands input back.

## Troubleshooting

- **Edge does nothing, tray icon yellow.** The router is waiting for the
  InputCapture consent dialog (GNOME shows one per router start). Find
  the dialog and approve it; a desktop notification points at it.
- **Edge does nothing, tray icon orange.** The router cannot reach the
  Pi bridge. Check the Pi is powered and on the network.
- **Input stutters after a cable change.** On the Pi, `dmesg | grep -c
  "new address"` climbing while idle means the USB link is resetting:
  replace the cable with a real data cable.
- **Input has a faint stutter, USB is clean.** Measure `ping` from the
  desktop to the Pi; a healthy 5GHz link sits under 10ms. Jitter with
  good signal usually means the Pi's antenna corner (by the USB-C port)
  has a cable or metal over it, or a 5GHz source is next to it. Ethernet
  on the Pi's eth0 ends the question.
- **Clipboard stopped syncing in both directions.** Restart the Linux
  agent: `systemctl --user restart beaglewing-clipboard`. It now detects
  a rebooted Pi within 30s (heartbeat) and re-reads the Xwayland auth
  cookie on every clipboard call, which covers the two ways this used to
  fail silently: a zombie connection after a Pi power cut, and a stale
  cookie after Xwayland restarted.
- **Stuck keys or buttons on the target.** Should not happen; every
  disconnect path releases everything. If it does, stop the router
  (`systemctl --user stop beaglewing-router`), which forces a release.
...
- The Pi's USB-A ports are host-only. Never use a USB-A male-to-male
  ...
  design.
- Use a proper USB data cable, not a charge-grade one. A marginal cable
  still powers the Pi and even enumerates, but the high-speed link
  resets every few seconds and input stutters. Diagnostic on the Pi:
  `dmesg | grep -c "new address"` climbing while idle means the cable
  (or a connector) is bad.
...
linux/           Rust input router, clipboard agent, status icon (one binary)
